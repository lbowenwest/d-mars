...
export function parseFile(contents: string[]): {
    ...
    const instructions = chunk(
        contents.slice(1).filter((c) => c.length > 0),
        2,
    ...
    return {
        world: { size: [worldSize[0], worldSize[1]], lostPositions: new Set() },
...
export function execute(filename: string) {
    ...
    const results = instructions.map(
        (instruction): Result => processInstruction(world, instruction),
    );
